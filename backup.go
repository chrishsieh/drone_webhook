	...
	if len(repoOwner) > 0 && len(repoName) > 0 {
		...
		if onSuccess == "change" || onFailure == "change" {
			...
			for page, foundLastBuild := 1, 0; page <= 4 && foundLastBuild == 0; page++ {
				if gotBuildlist, err := client.BuildList(repoOwner, repoName, drone.ListOptions{page}); err == nil {
		...
		if showNotify > 0 {
			if gotBuild, err := client.Build(repoOwner, repoName, buildNumber); err == nil {
